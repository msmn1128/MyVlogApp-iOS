...
/// 素直に扱えるようにする。
///
/// 可変状態を持たない値型なので、プロジェクト全体の既定（SWIFT_DEFAULT_ACTOR_ISOLATION
/// = MainActor）から明示的に外してある。外しておかないとEquatable適合までMainActor隔離に
/// なり、@MainActorでない単体テスト（WaveformGeometryTests）から`==`が使えない
/// （Swift 6モードではエラー。Models.swift/VlogLayout.swiftと同じ理由）。
...
/// nonisolatedにしてある理由は`WaveformViewport`と同じ。
nonisolated struct WaveformGeometry {
    let left:  CGFloat
    let right: CGFloat
...
// fitViewport（nonisolated）から読むので、こちらもnonisolatedにしておく
...
private nonisolated let fitFullThreshold: Double = 0.6
/// ズーム時、選択範囲の前後に確保する余白（選択範囲の長さに対する比率）
private nonisolated let fitMarginRatio: Double = 0.5
/// ズーム時に確保する余白の下限。選択範囲が短すぎても手がかりが残るように
private nonisolated let fitMinMarginMs: Int64 = 300
/// ズーム時の表示幅の下限。選択範囲がごく短くても波形が潰れないように
private nonisolated let fitMinWindowMs: Int64 = 3_000
